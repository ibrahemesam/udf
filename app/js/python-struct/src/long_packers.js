...
const UNPACK_UINT64_LE = (data, pos) =>
  Long.fromBits(data.readInt32LE(pos), data.readInt32LE(pos + 4), true);
const UNPACK_UINT64_BE = (data, pos) =>
  Long.fromBits(data.readInt32BE(pos + 4), data.readInt32BE(pos), true);
const UNPACK_INT64_LE = (data, pos) =>
  Long.fromBits(data.readInt32LE(pos), data.readInt32LE(pos + 4), false);
const UNPACK_INT64_BE = (data, pos) =>
  Long.fromBits(data.readInt32BE(pos + 4), data.readInt32BE(pos), false);

const PACK_INT64_LE = (data, pack, pos) => {
  if (!(data instanceof Long)) {
    if (typeof data === "number") data = Long.fromNumber(data);
